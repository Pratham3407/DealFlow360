    ...
    items: [
      {
        label: "Approvals",
        to: "/approvals",
        anyOf: ["approvals:read-queue"],
        status: "planned",
      },
      {
        label: "Deal health",
        to: "/deal-health",
        anyOf: ["deal-health:view"],
        status: "planned",
      },
    ],
  },
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        label: "Audit log",
        to: "/audit",
        anyOf: ["audit:view-all", "audit:view-own"],
        status: "planned",
      },
    ],
  },
...
export function visibleSections(
  ...
  return INTERNAL_NAV.map((section) => ({
    ...
    items: section.items.filter(
      (item) => item.anyOf.length === 0 || item.anyOf.some(can),
    ),
