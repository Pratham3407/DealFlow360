# DealFlow360 — RBAC

## Permission matrix

| Capability | Admin | Sales Rep | Manager | Finance/Ops | Customer |
|---|---:|---:|---:|---:|---:|
| Configure products | Yes | No | Optional | No | No |
| Configure price lists | Yes | No | Optional | No | No |
| Configure discount rules | Yes | No | Yes | No | No |
| Configure approval chains | Yes | No | Yes | No | No |
| Configure warehouses (create, stock levels, replenishment, shipping weighting) | Yes | No | No | No | No |
| Configure subscription plans (intervals, proration/cancellation/refund rules) | Yes | No | No | No | No |
| Create quotations | Yes | Yes | Optional | No | No |
| Edit quotations | Yes | Yes | Review/return | Limited | Request only |
| Apply discounts | Yes | Yes | Review | Review | Counter proposal |
| View margin | Yes | Yes | Yes | Yes | No |
| Approve manager-level | No/Optional | No | Yes | No | No |
| Approve finance-level | No/Optional | No | No | Yes | No |
| Accept/override warehouse split, decide backorder handling (operational, not config) | Yes | Track | View | Yes | View limited |
| Reconcile recurring billing, issue credit notes (operational, not config) | Yes | View | View | Yes | No |
| Negotiate | No | Respond | Review | Review | Yes |
| Confirm quotation | Yes | On behalf if authorized | Optional | Optional | Yes |
| View all analytics | Yes | Limited | Yes | Relevant | No |
| View audit logs | Yes | Own/authorized | Yes | Relevant | No |

## Configuration vs. operations (source clarification)

Per the source specification, Admin owns *setup* (warehouses, stock rules, shipping weighting, subscription plans) as backend configuration (PDF §A4–A5). Finance/Operations owns the *operational* decisions made during an active deal — accepting or overriding a fulfillment split, deciding backorder handling, reconciling recurring billing and credit notes (PDF §3, Finance/Operations role). These are distinct capabilities and must not share a single permission flag in the API layer.

## Portal isolation

Customer API responses must be filtered by authenticated customer identity.

Example:

```text
GET /portal/quotations/Q1001
```

The server must verify:

```text
quotation.customer_id == authenticated_customer.id
```

Never rely on frontend route protection alone.
