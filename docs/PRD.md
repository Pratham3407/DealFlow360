...
> **Implementation deviation — internal sign-up is admin-provisioned.**
>
> Open self-signup would let an anonymous caller create an account that sits
> outside the RBAC model of §21, and would leave the role of a brand-new account
> undefined. Accounts are therefore created by an ADMIN through
> `POST /api/users`, which requires the `users:manage` capability. Login itself is
> unchanged. Revisit if a genuine self-service tenant-onboarding requirement
> appears.
...
> **Implementation status.** Email + password is implemented, through a separate
> `POST /api/portal/auth/login` endpoint that refuses internal credentials (and
> whose internal counterpart refuses customer credentials). Magic-link delivery
> is not implemented; the session mechanism is a server-side token store, so
> adding it later means issuing a single-use token rather than reworking auth.
